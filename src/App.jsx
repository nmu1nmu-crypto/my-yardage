...
export default function App() {
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column",
      background: "hsl: 0%, 0%, 0%", color: "#F5F1E6" }}>
      <main style={{ flex: 1, overflow: "auto" }}>{screens[tab]}</main>
        ...
        {TABS.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            ...
            <span style={{ fontSize: 10.5, fontWeight: tab === t.id ? 700 : 600,
              color: tab === t.id ? "#C9A94E" : "rgba(245,241,230,0.5)" }}>
              {t.label}
            </span>
          </button>
        ))}
...
function TabIcon({ name, active }) {
  const col = active ? "#C9A94E" : "rgba(245,241,230,0.5)";
  const paths = {
